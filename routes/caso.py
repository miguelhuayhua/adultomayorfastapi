...
from database.Citacion import allCitacionByCaso, insertCitacion, suspenderCitacion, terminarCitacion
...
@routerCaso.post('/citacion/terminar')
async def allCitacion(request:Request):
    data = await request.json()
    id_citacion = data.get('id_citacion')
    citacion = await terminarCitacion(id_citacion)
    session.close()
    if(citacion):
        return {"status":1}
    else:
...
async def generarActaCompromiso(request:Request):
    try:
        data = await request.json()
        id_usuario = data.get('usuario').get('id_usuario')
        if(await generaActaCompromiso(data.get('id_caso')) ):
