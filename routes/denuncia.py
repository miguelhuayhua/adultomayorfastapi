...
async def insertDenuncia(request: Request):
    try:
        ...
        caso = await insertCaso(datosDenuncia, id_adulto)
        await setAccionUsuario(id_usuario, 'caso','CREATE')
        datosDenunciado = data.get('datosDenunciado')
        await insertDenunciado(datosDenunciado, caso['id_caso'])
        session.close()
        return {"response": "Los datos se han registrado correctamente...", "status": 1, "nro_caso":caso['nro_caso']}
