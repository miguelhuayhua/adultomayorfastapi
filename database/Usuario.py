...
async def cambiarEstado(id_usuario):
    ...
    if usuario.estado ==0:
        ...
        usuario.estado= 0
    usuario.ult_modificacion = datetime.datetime.now()
    session.commit()
    return True
...
async def modificarUsuario(usuario):
    ...
    usuarioUpdated.ult_modificacion = datetime.datetime.now()
    session.commit()
...
async def getAccesosById(id_usuario):
    horas = session.execute(f"SELECT * FROM total_horas_acceso_usuario('{id_usuario}')").first()
...
async def logOut(id_usuario):
    print(id_usuario)
    try:
        usuarioUpdated = session.query(Acceso).filter_by(id_usuario = id_usuario).order_by(desc(Acceso.fecha_hora_acceso)).first()
