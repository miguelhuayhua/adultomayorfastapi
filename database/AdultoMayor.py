...
async def cambiarEstado(id_adulto):
    ...
    if adulto.estado ==0:
        ...
        adulto.estado= 0
    adulto.ult_modificacion = datetime.datetime.now()
    session.commit()
    return True
async def modificarAdulto(adulto):
    ...
    materno = adulto.get('materno').strip().upper()
    paterno = adulto.get('paterno').strip().upper()
    nombre = adulto.get('nombre').strip().upper()
    complemento = adulto.get('complemento')
    ...
    adultoUpdated.complemento = complemento
    adultoUpdated.beneficios = beneficios
    adultoUpdated.ocupacion = ocupacion
    adultoUpdated.expedido = expedido
